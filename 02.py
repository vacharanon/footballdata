...
X['Predict'] = ''
os.makedirs(f'decision_tree/{league}/2017/{team}/', exist_ok=True)
for key, X_train_opponent in X_train_opponents:
    ...
    tree.export_graphviz(clf_entropy, out_file=f'decision_tree/{league}/2017/{team}/{key}.dot',
                         feature_names=X_test_opponent.columns.values,
                         class_names=clf_entropy.classes_)
    call(['dot', '-Tpng', f'decision_tree/{league}/2017/{team}/{key}.dot', '-o', f'decision_tree/{league}/2017/{team}/{key}.png'])

X['Actual'] = Y['Result']
x = X[X['Predict'] != '']
x.to_csv(f'./decision_tree/{league}/2017/{team}/x.csv')
print("Accuracy is ", accuracy_score(x['Actual'], x['Predict'])*100)
